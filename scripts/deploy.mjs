...
if (
  ![
    "https://github.com/diegocrisafu/diego_ux-ui.git",
    "git@github.com:diegocrisafu/diego_ux-ui.git",
  ].includes(remote)
) {
  throw new Error(
...
run("npm", ["test"]);
run("npm", ["run", "build"], {
  env: { ...process.env, VITE_BASE_PATH: "/diego_ux-ui/" },
});
...
const targets = [
  {
    remote,
    branch: "gh-pages",
    url: "https://diegocrisafu.github.io/diego_ux-ui/",
  },
];
for (const target of targets) {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "crowdflow-deploy-"));
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          sourceRepository: "https://github.com/diegocrisafu/diego_ux-ui",
          sourceCommit: source,
    ...
    if (read(["status", "--porcelain"], checkout))
      run(
        "git",
        ["commit", "-m", `Publish CrowdFlow from ${source.slice(0, 12)}`],
        { cwd: checkout },
      );
